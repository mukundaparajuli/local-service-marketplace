...
const ProviderCard = ({ provider }: { provider: any }) => {
    ...
    return (
        <div className="bg-white rounded-xl shadow-md hover:shadow-xl border border-gray-200 transition-shadow duration-300 max-w-sm">
            {/* Header section */}
            <div className="h-32 bg-gradient-to-r from-gray-700 to-gray-800 relative">
                <div className="absolute bottom-0 left-0 w-full p-4 bg-gradient-to-t from-black/50 to-transparent">
                    <h3 className="text-white font-semibold text-lg">{provider.businessName}</h3>
            ...
            <div className="p-4 text-sm text-gray-700">
                ...
                <div className="grid grid-cols-2 gap-3 mb-4">
                    <div className="flex items-center">
                        <Home size={16} className={`mr-2 ${provider.acceptsHomeVisits ? 'text-green-600' : 'text-gray-400'}`} />
                        <span className={`${provider.acceptsHomeVisits ? 'text-gray-800' : 'text-gray-400'}`}>
                            {provider.acceptsHomeVisits ? 'Home visits' : 'No home visits'}
                        </span>
                    </div>
                    <div className="flex items-center">
                        <Store size={16} className={`mr-2 ${provider.hasPhysicalStore ? 'text-green-600' : 'text-gray-400'}`} />
                        <span className={`${provider.hasPhysicalStore ? 'text-gray-800' : 'text-gray-400'}`}>
                ...
                <div className="flex justify-between items-center pt-3 border-t border-gray-100">
                    ...
                    <div className="flex items-center">
                        <Shield size={16} className={`mr-1 ${provider.isKYCVerified ? 'text-green-600' : 'text-gray-400'}`} />
                        <span className={`${provider.isKYCVerified ? 'text-green-600' : 'text-gray-400'}`}>
                            {provider.isKYCVerified ? 'Verified' : 'Unverified'}
                        </span>
                    </div>
                </div>

                <button className="mt-5 w-full bg-gray-700 hover:bg-gray-800 text-white py-2 px-4 rounded-md transition-colors text-sm font-medium"
                    onClick={() => {
